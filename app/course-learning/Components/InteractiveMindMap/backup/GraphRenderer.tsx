...
const GraphRenderer: React.FC<GraphRendererProps> = ({ 
  ...
  useEffect(() => {
    ...
    graphData.nodes.forEach((graphNode) => {
      ...
      if (isVisible) {
        // Prefer calculated position. User-dragged positions are stored in nodePositionsRef,
        // but resetLayout clears this. onNodeDragStop updates this ref.
        // If a node was dragged, its position in nodePositionsRef would be used *until* a layout recalculation.
        // The new model is: calculatedPositions are authoritative unless a node was *just* dragged.
        // Let's simplify: always use calculated positions here. Dragging sets `userInteracted`.
        const position = calculatedPositions.get(graphNode.id) || 
                         nodePositionsRef.current.find(cn => cn.id === graphNode.id)?.position || 
                         { x: 0, y: 0 };
    ...
    setEdges, 
    onNodePositionChange, 
    userInteracted, // Re-run if user interaction state changes (e.g., after a drag then a reset)
    isParentInitialized,
