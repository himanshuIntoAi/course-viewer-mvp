...
interface GraphRendererProps {
  ...
  // View-only mode disables editing interactions and hides non-view controls
  readOnly?: boolean;
}
...
const calculateNodePositions = (nodes: GraphData['nodes'], links: GraphData['links'], collapsedNodes: Set<string>) => {
  ...
  if (!nodes.some(n => n.id === rootId)) {
    // If provided data doesn't include id '1', fallback to the first node as root
    const first = nodes[0];
    if (first) {
      computeSubtreeSize(first.id);
    }
  } else {
  ...
  if (nodes.length > 0) {
    const actualRootId = nodes.some(n => n.id === rootId) ? rootId : nodes[0].id;
    positionSubtree(actualRootId, ROOT_X, 0);
  }
...
const GraphRendererLR: React.FC<GraphRendererProps> = ({
  ...
  // Local settings state initialized from props
  type ThemeMode = 'light' | 'dark' | 'auto';
  const [themeMode, setThemeMode] = useState<ThemeMode>('auto');
  const [localLineStyle, setLocalLineStyle] = useState<typeof lineStyle>(lineStyle);
  const [localLineCurveStyle, setLocalLineCurveStyle] = useState<typeof lineCurveStyle>(lineCurveStyle);
  const [localLineColorMode, setLocalLineColorMode] = useState<typeof lineColorMode>(lineColorMode);
  const [localCustomLineColor, setLocalCustomLineColor] = useState<string>(customLineColor);

  // Resolve canvas theme from themeMode and system preference
  const [resolvedCanvasTheme, setResolvedCanvasTheme] = useState<typeof canvasTheme>(canvasTheme);
  useEffect(() => {
    const applyTheme = () => {
      if (themeMode === 'auto') {
        const prefersDark = typeof window !== 'undefined' && window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
        setResolvedCanvasTheme(prefersDark ? 'dark' : 'light');
      } else {
        setResolvedCanvasTheme(themeMode);
      }
    };
    applyTheme();
  ...
  // Update the global lineStyle used by edge components
  ...
  const callbacksRef = useRef({
    ...
    toggleNode: (id: string) => {
      ...
      if (document.activeElement &&
        ...
        console.log(`[GraphRendererLR] Toggle ignored because AI generation button has focus for node: ${id}`);
        return;
      }
  ...
  useEffect(() => {
    ...
    graphData.nodes.forEach((node) => {
      ...
      if (isVisible) {
        const cachedNodeInfo = nodePositionsRef.current.find(cn => cn.id === node.id);
        let position;

        if (cachedNodeInfo && userInteracted) {
          position = cachedNodeInfo.position;
          console.log(`[GraphRendererLR] Using CACHED/DRAGGED position for node ${node.id}:`, position);
        } else {
          position = calculatedPositions.get(node.id) || { x: 0, y: 0 };
          if (cachedNodeInfo && !userInteracted) {
            // If user hasn't interacted, but we have a cache, it means it was from a previous layout calculation.
            // If calculatedPositions gives something different, it implies a structural change, so prefer calculated.
            // However, if they are the same, it's fine to log we're using the (effectively) calculated one.
            console.log(`[GraphRendererLR] Using CALCULATED (or cached if same) position for node ${node.id} (userInteracted: false):`, position);
          } else if (!cachedNodeInfo) {
            console.log(`[GraphRendererLR] Using CALCULATED position for NEW/RESET node ${node.id}:`, position);
          }
        }
    ...
    const reactFlowEdges: Edge[] = graphData.links
      ...
      .map(link => {
        ...
        const pc = localLineColorMode === "random" ? generateRandomColor() :
          localLineColorMode === "custom" ? localCustomLineColor :
        ...
        const edgeBase: Edge = {
          ...
          source: link.source,
          target: link.target,
          type: localLineCurveStyle === 'straight' ? 'straight' : 'custom',
          animated: localLineStyle === 'animated',
          style: {
            stroke: pc,
            strokeOpacity: 0.8,
            strokeWidth: 2,
            strokeDasharray: localLineStyle === 'dashed' || localLineStyle === 'dashed-arrow' ? '5,5' : undefined,
          },
          data: {
            pathColor: pc,
            style: {
              ...
              strokeWidth: 2,
              strokeDasharray: localLineStyle === 'dashed' || localLineStyle === 'dashed-arrow' ? '5,5' : undefined,
              animation: localLineStyle === 'animated' ? 'dashdraw 0.5s linear infinite' : undefined,
            }
          }
        };

        // Add isArrow flag for dashed-arrow style
        if (localLineStyle === 'dashed-arrow') {
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    const animated = localLineStyle === 'animated';
    ...
    const markerEnd = localLineStyle === 'dashed-arrow' ? 'url(#custom-arrow)' : undefined;
  ...
  const onEdgeUpdate = useCallback((oldEdge: Edge, newConnection: Connection) => {
    ...
    // Create the marker end for arrow with explicit dimensions
    const marker = localLineStyle === 'dashed-arrow' ? 'url(#custom-arrow)' : undefined;
  ...
  return (
    <div
      ...
      style={{
        ...
        width: '100%',
        position: 'relative',
        background: resolvedCanvasTheme === 'dark' ? '#000000' : '#FFFFFF',
      }}
      onContextMenu={handleContextMenu}
      className='relative'
    >
      <div className='flex absolute top-0 right-[5vw]' >
        <button className='flex border-2 border-gray-50 bg-white rounded-md p-2 mr-2 mt-2 items-center' onClick={() => setIsSettingsOpen(true)} > <Image src={"/mindmap/settingIcon.svg"} alt="setting icon" width={30} height={30} className='mr-2' /> Setting</button>
        <button className='flex border-2 border-gray-50 bg-white rounded-md p-2 mr-2 mt-2 items-center' onClick={() => setIsDownloadOpen(true)} > <Image src={"/mindmap/downloadIcon.svg"} alt="download icon" width={30} height={30} className='mr-2' /> Download</button>
        <button className='flex border-2 border-gray-50 bg-white rounded-md p-2 mr-2 mt-2 items-center' > <Image src={"/mindmap/frameIcon.svg"} alt="setting icon" width={30} height={30} className='mr-2' /> Verticle</button>
      </div>
      {linkMode && linkSource && (
        <div style={{
      ...
      <div className='absolute top-[30vh] right-[5vw] flex flex-col gap-2 ' >
        <button className='border-2 border-gray-50 bg-white rounded-md p-2' ><Image src={"/mindmap/aiSparkleIcon.svg"} alt="theme icon" width={30} height={30} /></button>
        <button className='border-2 border-gray-50 bg-white rounded-md p-2' ><Image src={"/mindmap/zoomInIcon.svg"} alt="layout icon" width={30} height={30} /></button>
        <button className='border-2 border-gray-50 bg-white rounded-md p-2' ><Image src={"/mindmap/fullPageIcon.svg"} alt="colour icon" width={30} height={30} /></button>
        <button className='border-2 border-gray-50 bg-white rounded-md p-2' ><Image src={"/mindmap/zoomOutIcon.svg"} alt="style icon" width={30} height={30} /></button>
      </div>
      ...
      {isDownloadOpen && (
        <div className='absolute top-[10vh] right-[25vh] w-[13vw] bg-white rounded-md border-2 border-gray-200 flex flex-col gap-2 z-[10000]' >
          <div className='flex justify-between items-center p-4 border-b-2 border-gray-200' >
            ...
            <button onClick={() => setIsDownloadOpen(false)} aria-label='Close download'>
              <Image src={"/mindmap/closeIcon.svg"} alt="download close icon" width={30} height={30} />
            </button>
          </div>
          <p className='p-4 cursor-pointer hover:bg-gray-100 border-b-2 border-gray-200' >PNG</p>
          <p className='p-4 cursor-pointer hover:bg-gray-100 border-b-2 border-gray-200' >JPEG</p>
          <p className='p-4 cursor-pointer hover:bg-gray-100 border-b-2 border-gray-200' >SVG</p>
          <p className='p-4 cursor-pointer hover:bg-gray-100 border-b-2 border-gray-200' >GIF</p>
          <p className='p-4 cursor-pointer hover:bg-gray-100 border-b-2 border-gray-200' >CSV</p>
          <p className='p-4 cursor-pointer hover:bg-gray-100 border-b-2 border-gray-200' >PDF</p>
          <p className='p-4 cursor-pointer hover:bg-gray-100 border-b-2 border-gray-200' >WORD</p>
          <p className='p-4 cursor-pointer hover:bg-gray-100' >HTML</p>
        </div>
      )}
    </div>
  );
