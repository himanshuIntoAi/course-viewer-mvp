...
const FRONTEND_URL = typeof window !== 'undefined' ? window.location.origin : process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
...
const makeRequest = async (url, options = {}) => {
  try {
    ...
    if (!url.includes('github.com')) {
      ...
      // If parent request is aborted, also abort health check
      // if (options.signal) {
      //   options.signal.addEventListener('abort', () => healthCheckController.abort());
      // }
      
      const isHealthy = await checkBackendHealth(/* healthCheckController.signal */);
      if (!isHealthy) {
    ...
    const requestOptions = {
      ...
      // signal: options.signal // Comment out abort signal
    };
    ...
    try {
      ...
      const response = await makeRequest(`${API_BASE}/auth/register`, {
        ...
        body: JSON.stringify({ 
          ...
          is_student: userType === 'student',  // true or false
          is_instructor: userType === 'instructor'  // true or false
        })
      });
...
export function generateOAuthState(redirectPath = '/') {
  ...
  console.log('Generated state object:', state); // Debug log
  return stateStr;
}
